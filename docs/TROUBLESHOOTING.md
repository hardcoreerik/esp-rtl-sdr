...
| Reboot loop a few seconds after boot on a non-Blog-V4 stick, `install v0.8.0-…` logs but device is never identified before the crash | `assert failed: usbh_dev_close usbh.c:1058 (dev_obj->dynamic.num_ctrl_xfers_inflight == 0)` — an EP0 STALL. **Fixed**: `probe_blog_v3_tuner()` was reading a tuner's I2C chip-id before the RTL2832U demod's own bring-up had run, which STALLs unconditionally (confirmed against a real Blog V4's own correct tuner address too, not specific to any one stick) and could cascade into this assert. `run_demod_bringup()` now runs first; see CHANGELOG "Fixed" for the full evidence trail. If you still see this on a current build, it's a new case — capture the same way (PC + pyusb, same two vendor commands, before vs. after replaying `kRtlInitTransfers[0..86)`) rather than assuming it's the same bug | Update to a build with the fix. As defense-in-depth against a *different* future incompatibility, the fault guard still latches after 3 consecutive enumeration-time panics and returns `ESP_RTL_SDR_ERR_USB_SAFE_MODE` from `install()` instead of retrying forever. To retry safely: unplug the receiver, call `esp_rtl_sdr_usb_fault_guard_reset()`, reconnect the receiver, then call `esp_rtl_sdr_install()` again. Unplugging or receiving a later `NEW_DEV` event alone does not clear the retained panic count. |
